  ...
  return (
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '28px' }}>
        <h1 style={{ fontSize: '26px', fontWeight: '900', color: 'var(--text-primary)', letterSpacing: '-0.6px' }}>API Access</h1>
        <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '4px' }}>Generate API keys to integrate AI Voice Engine into your apps</p>
      </motion.div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '16px', marginBottom: '20px' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} style={card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '18px' }}>
            ...
            <p style={{ fontSize: '14px', fontWeight: '800', color: 'var(--text-primary)' }}>Monthly Usage</p>
            {usage && (
              <span style={{ marginLeft: 'auto', fontSize: '11px', fontWeight: '700', padding: '3px 10px', borderRadius: '999px', background: 'var(--accent-bg)', color: 'var(--accent-light)', textTransform: 'uppercase' }}>
                {usage.plan}
              </span>
            )}
          </div>
          {usage && (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Calls this month</span>
                <span style={{ fontSize: '12px', fontWeight: '700', color: 'var(--text-primary)' }}>{usage.calls_this_month} / {usage.calls_limit}</span>
              </div>
              <div style={{ height: '8px', background: 'var(--border)', borderRadius: '999px', overflow: 'hidden', marginBottom: '12px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px' }}>
                {[
                  ...
                  { label: 'API Keys', value: `${usage.api_keys_active}/${usage.api_keys_limit}`, color: 'var(--accent-light)' },
                  { label: 'Total Ever', value: usage.total_calls_ever, color: '#06b6d4' },
                ].map(s => (
                  <div key={s.label} style={{ background: 'var(--bg-input)', borderRadius: '10px', padding: '10px', textAlign: 'center' }}>
                    <p style={{ fontSize: '18px', fontWeight: '800', color: s.color }}>{s.value}</p>
                    <p style={{ fontSize: '10px', color: 'var(--text-dim)', marginTop: '2px' }}>{s.label}</p>
                  </div>
                ))}
              </div>
              {pct >= 80 && (
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} style={card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '18px' }}>
            ...
            <p style={{ fontSize: '14px', fontWeight: '800', color: 'var(--text-primary)' }}>Generate New Key</p>
          </div>
          <input value={newKeyName} onChange={e => setNewKeyName(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleCreate()}
            placeholder="Key name (e.g. Production, Mobile App)"
            style={{ width: '100%', padding: '10px 12px', borderRadius: '10px', background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-primary)', fontSize: '12px', outline: 'none', boxSizing: 'border-box', marginBottom: '12px' }} />
          ...
          <div style={{ marginTop: '14px', background: 'rgba(6,182,212,0.06)', border: '1px solid rgba(6,182,212,0.12)', borderRadius: '10px', padding: '10px 12px' }}>
            ...
            <p style={{ fontSize: '10px', color: 'var(--text-dim)', fontFamily: 'monospace' }}>Header: x-api-key: tzm_live_xxxx</p>
            <p style={{ fontSize: '10px', color: 'var(--text-dim)', fontFamily: 'monospace', marginTop: '2px' }}>Or: x-client-id: your_client_id</p>
          </div>
        </motion.div>
      </div>
        ...
        {newKey && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
              ...
              <button onClick={() => setNewKey(null)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '16px' }}>✕</button>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--bg-input)', borderRadius: '10px', padding: '12px 14px' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} style={card}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '18px' }}>
          ...
          <p style={{ fontSize: '14px', fontWeight: '800', color: 'var(--text-primary)' }}>Your API Keys</p>
          <span style={{ marginLeft: 'auto', fontSize: '11px', color: 'var(--text-dim)' }}>{keys.length} key{keys.length !== 1 ? 's' : ''}</span>
        </div>

        {loading ? (
          <p style={{ fontSize: '12px', color: 'var(--text-dim)', textAlign: 'center', padding: '20px' }}>Loading...</p>
        ) : keys.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <Key size={28} style={{ color: 'var(--border)', margin: '0 auto 10px' }} />
            <p style={{ fontSize: '13px', color: 'var(--text-dim)' }}>No API keys yet</p>
            <p style={{ fontSize: '11px', color: 'var(--text-dim)', marginTop: '4px' }}>Generate your first key above</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {keys.map((k, i) => (
              <motion.div key={k.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', background: 'var(--bg-input)', borderRadius: '12px', border: `1px solid ${k.is_active ? 'var(--accent-border)' : 'var(--border)'}` }}>
                <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: k.is_active ? '#10b981' : 'var(--text-dim)', boxShadow: k.is_active ? '0 0 6px #10b981' : 'none', flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '13px', fontWeight: '700', color: 'var(--text-primary)' }}>{k.name}</p>
                  <p style={{ fontSize: '11px', color: 'var(--text-dim)', fontFamily: 'monospace', marginTop: '2px' }}>{k.key_preview}</p>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <p style={{ fontSize: '12px', fontWeight: '700', color: 'var(--text-primary)' }}>{k.calls_used} <span style={{ color: 'var(--text-dim)', fontWeight: '400' }}>/ {k.calls_limit}</span></p>
                  <p style={{ fontSize: '10px', color: 'var(--text-dim)' }}>{k.last_used ? `Last: ${new Date(k.last_used).toLocaleDateString()}` : 'Never used'}</p>
                </div>
                <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                  <button onClick={() => handleToggle(k.id)} title={k.is_active ? 'Disable' : 'Enable'}
                    style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '8px', padding: '6px 8px', cursor: 'pointer', color: k.is_active ? '#10b981' : 'var(--text-dim)' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
        ...
        <p style={{ fontSize: '13px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '14px' }}>Quick API Reference</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '8px' }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 12px', background: 'var(--bg-input)', borderRadius: '8px' }}>
              <span style={{ fontSize: '9px', fontWeight: '800', padding: '2px 7px', borderRadius: '5px', background: e.method === 'GET' ? 'rgba(6,182,212,0.15)' : 'var(--accent-bg)', color: e.method === 'GET' ? '#06b6d4' : 'var(--accent-light)', flexShrink: 0 }}>{e.method}</span>
              <code style={{ fontSize: '11px', color: 'var(--text-primary)', flex: 1 }}>{e.path}</code>
              <span style={{ fontSize: '10px', color: 'var(--text-dim)' }}>{e.desc}</span>
            </div>
          ))}
        </div>
        <a href="/docs" target="_blank" style={{ display: 'inline-block', marginTop: '12px', fontSize: '12px', color: 'var(--accent-light)', textDecoration: 'none' }}>
