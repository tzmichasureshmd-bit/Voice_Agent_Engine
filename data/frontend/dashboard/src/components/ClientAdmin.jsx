  ...
  return (
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '26px', fontWeight: '900', color: 'var(--text-primary)', letterSpacing: '-0.6px' }}>Admin Panel</h1>
        <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '4px' }}>Manage your team, usage and account</p>
      </motion.div>
      ...
      <div style={{ display: 'flex', gap: '4px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '4px', marginBottom: '20px', width: 'fit-content' }}>
        {[['team', 'Team Members'], ['usage', 'Usage & Plan']].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)}
            style={{ padding: '7px 18px', borderRadius: '9px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: '700', background: tab === id ? '#7c3aed' : 'transparent', color: tab === id ? '#fff' : 'var(--text-muted)', transition: 'all 0.15s' }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {tab === 'team' && (
        <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>{team.length} member{team.length !== 1 ? 's' : ''}</p>
          ...
          {showForm && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} style={{ ...card, marginBottom: '16px', border: '1px solid rgba(124,58,237,0.3)' }}>
              <p style={{ fontSize: '13px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '16px' }}>New Team Member</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                {[['name','Name','text'],['email','Email','email'],['password','Password','password']].map(([key, ph, type]) => (
                  <input key={key} type={type} value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                    placeholder={ph} style={{ padding: '9px 12px', borderRadius: '9px', background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-primary)', fontSize: '12px', outline: 'none' }} />
                ))}
                <select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}
                  style={{ padding: '9px 12px', borderRadius: '9px', background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-primary)', fontSize: '12px', outline: 'none' }}>
              ...
              {/* Page Permissions */}
              <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '8px', fontWeight: '600' }}>Page Access</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '14px' }}>
                {PAGES.map(page => {
                  const active = form.permissions.split(',').includes(page)
                  return (
                    <button key={page} onClick={() => togglePerm(page)}
                      style={{ padding: '4px 10px', borderRadius: '6px', border: `1px solid ${active ? 'rgba(124,58,237,0.4)' : 'var(--border)'}`, background: active ? 'rgba(124,58,237,0.15)' : 'transparent', color: active ? '#a78bfa' : 'var(--text-dim)', fontSize: '11px', fontWeight: '600', cursor: 'pointer', textTransform: 'capitalize' }}>
                      {page}
                    </button>
                  )
                })}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            team.length === 0 ? (
              <div style={{ ...card, textAlign: 'center', padding: '40px' }}>
                <Users size={28} style={{ color: 'var(--border)', margin: '0 auto 10px' }} />
                <p style={{ fontSize: '13px', color: 'var(--text-dim)' }}>No team members yet</p>
                <p style={{ fontSize: '11px', color: 'var(--border)', marginTop: '4px' }}>Add your first team member above</p>
              </div>
            ) : team.map((m, i) => {
              ...
              return (
                <motion.div key={m.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: '13px', fontWeight: '700', color: m.is_active ? 'var(--text-primary)' : 'var(--text-dim)' }}>{m.name}</p>
                    <p style={{ fontSize: '11px', color: 'var(--text-dim)', marginTop: '1px' }}>{m.email}</p>
                  </div>
      ...
      {tab === 'usage' && usage && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} style={card}>
            <p style={{ fontSize: '14px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '16px' }}>📊 Monthly Usage</p>
            ...
            <div style={{ height: '10px', background: 'var(--border)', borderRadius: '999px', overflow: 'hidden', marginBottom: '16px' }}>
            ...
            {[
              ...
              <div key={s.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--bg-card)' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} style={card}>
            <p style={{ fontSize: '14px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '16px' }}>💎 Current Plan</p>
            <div style={{ background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: '12px', padding: '16px', marginBottom: '14px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '4px' }}>₹{(usage.plan_price/1000).toFixed(0)}K / month</p>
            </div>
