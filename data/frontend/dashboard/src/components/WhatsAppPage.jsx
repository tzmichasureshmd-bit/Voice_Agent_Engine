    ...
    <div>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: '900', color: '#f0f0f8', letterSpacing: '-0.5px' }}>WhatsApp</h1>
            <p style={{ fontSize: '12px', color: '#55556a' }}>Send messages & call summaries via WhatsApp</p>
          </div>
        </div>
      </motion.div>
      ...
      <div style={{ display: 'flex', gap: '4px', background: '#0e0e1a', border: '1px solid #1e1e30', borderRadius: '12px', padding: '4px', marginBottom: '20px', width: 'fit-content' }}>
        {[['broadcast','📢 Broadcast'],['templates','📋 Templates'],['auto','⚡ Auto-Summary']].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)}
            style={{ padding: '7px 16px', borderRadius: '9px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: '700', background: tab === id ? '#25d366' : 'transparent', color: tab === id ? '#fff' : '#55556a', transition: 'all 0.15s' }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {tab === 'broadcast' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} style={card}>
            <p style={{ fontSize: '13px', fontWeight: '800', color: '#f0f0f8', marginBottom: '14px' }}>Send Message</p>
            <div style={{ marginBottom: '12px' }}>
              <label style={{ fontSize: '11px', color: '#55556a', fontWeight: '600', display: 'block', marginBottom: '6px' }}>Phone Numbers (one per line)</label>
              <textarea value={numbers} onChange={e => setNumbers(e.target.value)}
                placeholder={"+91 9876543210\n+91 8765432109\n..."}
                style={{ width: '100%', height: '120px', padding: '10px 12px', borderRadius: '10px', background: '#0a0a14', border: '1px solid #1e1e30', color: '#f0f0f8', fontSize: '12px', outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'monospace' }} />
            </div>
            <div style={{ marginBottom: '14px' }}>
              <label style={{ fontSize: '11px', color: '#55556a', fontWeight: '600', display: 'block', marginBottom: '6px' }}>Message</label>
              <textarea value={message} onChange={e => setMessage(e.target.value)}
                placeholder="Type your message here..."
                style={{ width: '100%', height: '100px', padding: '10px 12px', borderRadius: '10px', background: '#0a0a14', border: '1px solid #1e1e30', color: '#f0f0f8', fontSize: '12px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
              <p style={{ fontSize: '10px', color: '#33334a', marginTop: '4px' }}>{message.length} characters</p>
            </div>
            <button onClick={handleSend} disabled={sending || !numbers.trim() || !message.trim()}
              style={{ width: '100%', padding: '11px', borderRadius: '10px', border: 'none', background: sending ? '#1e1e30' : 'linear-gradient(135deg,#25d366,#128c7e)', color: 'white', fontSize: '13px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', opacity: (!numbers.trim() || !message.trim()) ? 0.5 : 1 }}>
            ...
            <p style={{ fontSize: '10px', color: '#33334a', marginTop: '8px', textAlign: 'center' }}>Opens WhatsApp Web for each number. Works without WhatsApp Business API.</p>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} style={card}>
            <p style={{ fontSize: '13px', fontWeight: '800', color: '#f0f0f8', marginBottom: '14px' }}>Quick Stats</p>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: i < 2 ? '1px solid #0e0e1a' : 'none' }}>
                <div>
                  <p style={{ fontSize: '13px', fontWeight: '600', color: '#f0f0f8' }}>{s.label}</p>
                  <p style={{ fontSize: '11px', color: '#33334a', marginTop: '2px' }}>{s.desc}</p>
                </div>
                <span style={{ fontSize: '10px', fontWeight: '700', padding: '3px 10px', borderRadius: '999px', background: s.status ? `rgba(37,211,102,0.1)` : 'rgba(85,85,106,0.1)', color: s.status ? '#25d366' : '#55556a' }}>
                  {s.status ? 'ON' : 'OFF'}
                </span>
              </div>
            ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '12px' }}>
          {TEMPLATES.map((tpl, i) => (
            <motion.div key={tpl.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              style={{ ...card, cursor: 'pointer', border: selectedTpl === tpl.id ? '1px solid rgba(37,211,102,0.4)' : '1px solid #1e1e30' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                <p style={{ fontSize: '13px', fontWeight: '700', color: '#f0f0f8' }}>{tpl.name}</p>
                {selectedTpl === tpl.id && <CheckCircle2 size={14} color="#25d366" />}
              </div>
              <p style={{ fontSize: '12px', color: '#55556a', lineHeight: 1.6, marginBottom: '12px' }}>{tpl.msg}</p>
      ...
      {tab === 'auto' && (
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} style={card}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
            ...
            <p style={{ fontSize: '14px', fontWeight: '800', color: '#f0f0f8' }}>Auto-Summary Configuration</p>
          </div>
          <div style={{ background: 'rgba(37,211,102,0.06)', border: '1px solid rgba(37,211,102,0.15)', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
            ...
            <p style={{ fontSize: '12px', color: '#55556a', lineHeight: 1.7 }}>
          ...
          <p style={{ fontSize: '12px', color: '#55556a', marginBottom: '12px' }}>Configure auto-summary settings in <strong style={{ color: '#a78bfa' }}>Settings → WhatsApp Auto-Summary</strong></p>
          <div style={{ display: 'flex', gap: '8px' }}>
            <div style={{ flex: 1, background: '#0a0a14', border: '1px solid #1e1e30', borderRadius: '10px', padding: '14px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '11px', color: '#33334a', marginTop: '4px' }}>Notify Numbers</p>
            </div>
            <div style={{ flex: 1, background: '#0a0a14', border: '1px solid #1e1e30', borderRadius: '10px', padding: '14px', textAlign: 'center' }}>
              <p style={{ fontSize: '22px', fontWeight: '900', color: localStorage.getItem('s_wa_auto') === 'true' ? '#25d366' : '#33334a' }}>
                {localStorage.getItem('s_wa_auto') === 'true' ? 'ON' : 'OFF'}
              </p>
              <p style={{ fontSize: '11px', color: '#33334a', marginTop: '4px' }}>Auto-Send</p>
            </div>
          </div>
        </motion.div>
      )}
